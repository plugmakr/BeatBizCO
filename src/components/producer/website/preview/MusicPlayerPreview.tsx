...
export const MusicPlayerPreview = ({ content }: MusicPlayerPreviewProps) => {
  ...
  return `
    <div class="fixed bottom-0 left-0 right-0 bg-black/90 backdrop-blur-lg border-t border-white/10 p-6">
      <div class="max-w-7xl mx-auto">
        <div class="flex items-center justify-between mb-6">
          <div class="flex items-center gap-4">
            <button class="rounded-full p-3 bg-white/10 hover:bg-white/20 text-white transition-all">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <polygon points="5 3 19 12 5 21 5 3"></polygon>
              </svg>
            </button>
            <div class="text-white">
              <h3 class="font-semibold text-lg">${tracks[0].title}</h3>
          ...
          <div class="flex items-center gap-4">
            ...
            <button class="flex items-center gap-2 bg-white text-black px-4 py-2 rounded-full text-sm font-medium hover:bg-white/90 transition-colors">
        ...
        <div class="grid grid-cols-5 gap-4">
          ${tracks.map((track, index) => `
            <div class="bg-white/5 p-4 rounded-lg hover:bg-white/10 transition-colors cursor-pointer group" 
              ...
              <h4 class="font-medium text-white group-hover:text-white/90">${track.title}</h4>
