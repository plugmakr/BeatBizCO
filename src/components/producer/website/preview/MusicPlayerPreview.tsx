    ...
    <div class="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-lg border-t border-white/10 p-4">
      <div class="max-w-7xl mx-auto flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div class="text-white">
            <a href="#/tracks/${content?.currentTrack?.id || ''}" class="hover:text-white/80">
              <h3 class="font-semibold">${content?.currentTrack?.title || "Select a track"}</h3>
            </a>
            <div class="flex items-center gap-2 text-sm text-white/60">
              <span>${content?.artistName || "Artist Name"}</span>
              ${content?.currentTrack?.bpm ? 
                `<span class="px-2 py-0.5 bg-white/10 rounded-full text-xs">${content?.currentTrack?.bpm} BPM</span>` : 
                ''
              }
              ${content?.currentTrack?.key ? 
                `<span class="px-2 py-0.5 bg-white/10 rounded-full text-xs">Key: ${content?.currentTrack?.key}</span>` : 
                ''
              }
            </div>
          </div>
      ...
      <div class="max-w-7xl mx-auto mt-4">
        <div class="grid grid-cols-5 gap-4">
          ${(content?.playlist || []).map((track: any) => `
